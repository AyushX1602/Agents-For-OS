...
function isWindowsDrivePath(resolved) {
  return process.platform === 'win32' && /^[A-Za-z]:[\\/]/.test(path.resolve(resolved))
}

// Returns true when resolved is exactly FS_ROOT or a strict descendant.
// On Windows, File Explorer's "This PC" exposes drive roots, so absolute drive
// paths are also valid local roots in single-user desktop mode.
function isInsideRoot(resolved) {
  const root = path.resolve(FS_ROOT)
  const target = path.resolve(resolved)
  return target === root || target.startsWith(root + path.sep) || isWindowsDrivePath(target)
}

// Throws 403-style Error when path escapes the root.
function assertInsideRoot(resolved, label) {
  if (!isInsideRoot(resolved)) {
    const err = new Error(`Access denied: ${label || 'path'} is outside the allowed filesystem roots`)
    err.status = 403
    throw err
  }
}
...
function getWindowsDrives() {
  ...
  try {
    ...
    output.split('\n').map(l => l.trim()).filter(l => /^[A-Z]:$/.test(l))
      .forEach(d => found.add(d.toUpperCase()))
  } catch {}

  // WMIC is absent on some newer Windows installs. Fall back to probing drive
  // roots directly so "This PC" still works.
  for (let code = 65; code <= 90; code++) {
    const drive = `${String.fromCharCode(code)}:`
    try {
      if (fs.existsSync(`${drive}\\`)) found.add(drive)
    } catch {}
  }
...
router.get('/read', (req, res) => {
  try {
    ...
    const allowedExts = ['.txt', '.md', '.json', '.js', '.py', '.html', '.css', '.xml', '.yaml', '.yml',
...
router.delete('/delete', scopePermissions, (req, res) => {
  try {
    ...
    // Never allow trashing the filesystem root itself — renameSync(FS_ROOT, trash/…)
    // would try to move the root into its own subdirectory (EINVAL on most OSes).
    if (path.resolve(fullPath) === path.resolve(FS_ROOT)) {
      return res.status(400).json({ error: 'Cannot delete the filesystem root' })
    }
    ...
    // Use the full id in the on-disk name so simultaneous deletes of same-named
    // files never collide (the id carries the random suffix).
    const trashedName = `${id}__${basename}`
...
router.post('/rename', scopePermissions, (req, res) => {
  try {
    ...
    // Assert source is inside root before even checking existence
    assertInsideRoot(fullPath, 'rename source')
    ...
    // Destination must also be inside root (uses shared helper for consistency)
    assertInsideRoot(newPath, 'rename destination')
    ...
    // Audit log (non-blocking)
    prisma.fileActivity.create({
      data: { path: newPath, action: 'rename', userName: req.session?.userName || 'anonymous' }
    }).catch(e => console.warn('[FileActivity] rename log failed:', e.message))
...
router.post('/copy', scopePermissions, (req, res) => {
  try {
    ...
    // Both source AND destination must be inside FS_ROOT.
    // Without the source check a caller could copy from /etc/passwd into FS_ROOT
    // then read it back via /read or /view — same class as C1/H3.
    assertInsideRoot(fullSource, 'copy source')
    assertInsideRoot(fullDest,   'copy destination')
...
// GET /activity?limit= — Recent file activity for the current session user
// Read-only: no scopePermissions needed. Wraps errors with sendErr.
router.get('/activity', async (req, res) => {
  try {
    ...
    // Return relative paths where possible (strip FS_ROOT prefix)
    const rootAbs = path.resolve(FS_ROOT)
    const items = rows.map(r => {
      let relPath = r.path
      try {
        if (r.path.startsWith(rootAbs)) {
          relPath = '/' + path.relative(rootAbs, r.path).replace(/\\/g, '/')
        }
      } catch (_) {}
      return { ...r, path: relPath }
    })
