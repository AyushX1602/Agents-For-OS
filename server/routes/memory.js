...
function getScopedMem(root, userName) {
  return (userName && root[userName] && typeof root[userName] === 'object' && !root[userName].value)
    ? root[userName]
    : (root._global || root)
}
...
router.post('/save', (req, res) => {
  try {
    ...
    bucket[category][key] = { value, ts: new Date().toISOString() }
    writeMem(root)
...
router.get('/recall', (req, res) => {
  try {
    ...
    const root = readMem()
    const userName = req.session?.userName
    const mem = getScopedMem(root, userName)
    ...
    if (key) {
      for (const [cat, entries] of Object.entries(mem)) {
        if (entries && typeof entries === 'object' && entries[key]) {
          return res.json({ category: cat, key: key, value: entries[key].value, ts: entries[key].ts })
        }
      }
