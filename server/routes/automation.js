...
const { requireAuth } = require('../middleware/auth')         // S2
const { isCommandSafe } = require('../lib/commandSafety')    // S2

// S2: All automation routes require a valid session.
// These routes execute shell commands — unauthenticated remote execution
...
router.post('/workflow/run', async (req, res) => {
  try {
    ...
    const result = await auto.runWorkflow(name, { sessionId: req.session?.userName || 'User' })
    res.json(result)
  } catch (err) {
...
router.post('/workflow', async (req, res) => {
  try {
    ...
    // S2: Validate any shell commands embedded in workflow actions
    for (const action of data.actions) {
      if (action.command) {
        const check = isCommandSafe(action.command)
        if (!check.safe) {
          return res.status(400).json({ error: `Workflow action command blocked: ${check.reason}` })
        }
      }
    }
...
router.post('/schedule', (req, res) => {
  try {
    ...
    // S2: Validate command against the shared allowlist before scheduling
    const check = isCommandSafe(data.command)
    if (!check.safe) {
      return res.status(400).json({ error: `Command blocked: ${check.reason}` })
    }
