...
// S3: Rate-limit all PIN-verification endpoints.
// 10 attempts / 15 min / IP — makes 4-digit PIN brute-force take ~10 hours per IP.
const vaultLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,  // 15 minutes
  max: 10,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'Too many vault attempts, please try again later' }
})
...
// ── Unlock / verify PIN ───────────────────────────────────────────────────────
router.post('/unlock', vaultLimiter, (req, res) => {
  try {
...
router.post('/entry/get', vaultLimiter, (req, res) => {
  try {
...
// ── Change PIN ────────────────────────────────────────────────────────────────
router.post('/change-pin', vaultLimiter, (req, res) => {
  try {
