...
router.post('/index', async (req, res) => {
  try {
    ...
    // Use path.sep boundary — startsWith(root) alone lets /home/user-evil
    // pass for root /home/user (sibling-directory escape).
...
// ── Spotlight: unified search across files + notes ───────────────────────────
// Merges semantic file search + notes/memory into one ranked list.
// Intentionally thin — delegates to existing vectorSearch + memory libs.
...
router.get('/spotlight', async (req, res) => {
  try {
    ...
    const top = Math.min(parseInt(limit) || 8, 20)

    // 1. Semantic file search
    let fileResults = []
    try {
      fileResults = await vs.search(q.trim(), top, 0.3)
    } catch (_) { /* index may be empty — that's fine */ }

    // 2. Notes search (simple in-memory text match via memory/notes files)
    let noteResults = []
    try {
      const notesDir = require('path').join(fsRoot(), 'SpiritOS Notes')
      const fs = require('fs')
      if (fs.existsSync(notesDir)) {
        const files = fs.readdirSync(notesDir).filter(f => f.endsWith('.txt'))
        for (const f of files) {
          const title = f.replace('.txt', '')
          if (title.toLowerCase().includes(q.trim().toLowerCase())) {
            noteResults.push({ name: title, path: require('path').join(notesDir, f), type: 'note', score: 0.8 })
          }
        }
      }
    } catch (_) {}
    ...
    const combined = [
      ...fileResults.map(r => ({ ...r, resultType: 'file' })),
      ...noteResults.map(r => ({ ...r, resultType: 'note' }))
    ].slice(0, top)

    res.json({ query: q.trim(), results: combined, needsIndex: fileResults.length === 0 })
  } catch (err) {
    console.error('[Search] spotlight error:', err.message)
    res.status(500).json({ error: err.message })
  }
})
