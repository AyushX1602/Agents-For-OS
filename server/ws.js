...
function initWatcher() {
  // Use the same root as fs.js so file-change events match what the UI shows
  const os = require('os')
  let watchRoot = process.env.FS_ROOT || process.env.DEMO_FS_ROOT || './demo-filesystem'
  if (!require('path').isAbsolute(watchRoot)) {
    // __dirname is <projectRoot>/server, so go up ONE level to reach the
    // project root where ./demo-filesystem lives. '../../' overshot to the
    // parent of the project root, making the relative fallback resolve to a
    // non-existent path (and silently fall back to watching the home dir).
    watchRoot = require('path').resolve(__dirname, '..', watchRoot)
  }
  if (!require('fs').existsSync(watchRoot)) {
    watchRoot = os.homedir()
  }
  ...
  watcher = chokidar.watch(watchRoot, {
    ignored: [
      /(^|[\/\\])(\.|node_modules)/,    // dotfiles + node_modules
      /AppData[\/\\]Local[\/\\]Microsoft[\/\\]WindowsApps/,  // protected Windows Store stubs
    ...
    ignoreInitial: true,
    depth: 4,
    ignorePermissionErrors: true   // suppress EACCES/EPERM as warnings, not crashes
  })
