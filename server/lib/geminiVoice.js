...
async function handleToolCall(session, toolCall) {
  ...
  // Scope user-specific tools (memory, reminders) to this connection's session
  // id, mirroring the text path in irisEngine which calls handler(args, context).
  // Without this, voice tool calls fell back to userName 'User'/legacy buckets,
  // so voice-created reminders/memories were invisible to the text engine.
  // The session id is registered on the WS via the 'identify' message.
  const context = { sessionId: session.clientWs?._sessionId }
  ...
  for (const fc of (toolCall.functionCalls || [])) {
    ...
    try {
      ...
      if (handler) {
        const result = await handler(fc.args || {}, context)
        responseData = { result }
