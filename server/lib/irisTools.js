 ...
 * Prevents path-traversal above the root AND sibling-directory escape
 * (e.g. /home/user-evil must not pass for root /home/user).
 * Mirrors the identical helper in routes/fs.js.
 */
...
// ── App name normalizer ──────────────────────────────────────────────────────
// LLMs often emit lowercase, abbreviated, or space-separated app names.
// This maps every reasonable variant to the canonical PascalCase key used
// by WindowFrame.jsx and the APPS registry.
const APP_NAME_MAP = {
  fileexplorer:   'FileExplorer', files:     'FileExplorer', filemanager: 'FileExplorer',
  file:           'FileExplorer', explorer:  'FileExplorer',
  ...
  knownbook:      'KnownBook',    contacts:  'KnownBook',    'known book': 'KnownBook',
...
function canonicalAppName(name = '') {
  ...
  return APP_NAME_MAP[key]
    || Object.keys(APP_NAME_MAP).find(k => key.startsWith(k))
        && APP_NAME_MAP[Object.keys(APP_NAME_MAP).find(k => key.startsWith(k))]
    || name  // passthrough if nothing matches
}
...
const handlers = {
  ...
  // ── System ──────────────────────────────────────────────────────────────
  // Canonical app name map — LLMs often emit lowercase or abbreviated names.
  // Normalizing here means every engine (Gemini, OpenRouter, Spirit) benefits.
  async open_app({ app_name }) {
    return { action: 'openApp', target: canonicalAppName(app_name) }
  },

  async close_app({ app_name }) {
    return { action: 'closeApp', target: canonicalAppName(app_name) }
  },

  async open_url({ url, query }) {
    // Build the URL to open.
    // The tool description asks the model to supply a full URL (e.g.
    // "https://www.youtube.com/results?search_query=..."), so `url` is
    // typically already correct. If only `query` is given, fall back to
    // a Google Search URL. Both cases end up in the default browser.
    let finalUrl = url || ''
    if (!finalUrl && query) {
    ...
    return { action: 'openUrl', target: 'Browser', url: finalUrl }
  },

  async run_terminal({ command }) {
    const { isCommandSafe } = require('./commandSafety')
    const check = isCommandSafe(command)
    if (!check.safe) {
      return { error: `Command blocked: ${check.reason}`, command }
    }
    try {
  ...
  // ── Memory (categorized persistence, per-user scoped) ───────────────────
  async save_memory({ category, key, value }, context) {
    const memPath = path.join(__dirname, '..', 'data', 'memory.json')
    let root = {}
    try { root = JSON.parse(fs.readFileSync(memPath, 'utf-8')) } catch (_) {}

    const userName = context?.sessionId
    // Per-user: store under root[userName][category][key]
    // Legacy: store under root[category][key] (single-user)
    const bucket = userName ? (root[userName] || (root[userName] = {})) : root
    const cat = category || 'notes'
    if (!bucket[cat]) bucket[cat] = {}
    bucket[cat][key] = { value, ts: new Date().toISOString() }

    fs.mkdirSync(path.dirname(memPath), { recursive: true })
    fs.writeFileSync(memPath, JSON.stringify(root, null, 2))
    return { saved: `${cat}/${key}` }
  },

  async recall_memory({ key, category }, context) {
    const memPath = path.join(__dirname, '..', 'data', 'memory.json')
    try {
      const root = JSON.parse(fs.readFileSync(memPath, 'utf-8'))
      const userName = context?.sessionId
      // Resolve user bucket (per-user or legacy)
      const mem = (userName && root[userName] && typeof root[userName] === 'object' && !root[userName].value)
        ? root[userName]
        : (root._global || root)
      ...
      if (category && mem[category]) {
        if (typeof mem[category] !== 'object' || mem[category] === null) {
      ...
      if (key) {
        for (const [cat, entries] of Object.entries(mem)) {
          if (entries && typeof entries === 'object' && entries[key]) {
            return { category: cat, key, value: entries[key].value }
          }
        }
...
const toolDeclarations = [
  ...
  {
    name: 'open_url',
    description: 'Open the Browser app and navigate to a URL, or search. Use for "open YouTube", "play X on YouTube", "search Google for X", "go to github.com", etc.',
    parameters: {
      type: 'OBJECT',
      properties: {
        url:   { type: 'STRING', description: 'Full URL to open, e.g. "https://youtube.com/results?search_query=lofi+music"' },
        query: { type: 'STRING', description: 'Search query — if provided without a url, opens Google Search for this term' }
      },
      required: []
    }
  },
