...
export const APPS = {
  ...
  // W1: Mail and KnownBook were missing from APPS — Spotlight and getDefaultSize
  // could not resolve them. They exist as full app components in WindowFrame.
  Mail: {
    name: 'Mail',
    icon: '✉️',
...
// PdfViewer, ImageViewer, Mail, and KnownBook are excluded from desktop icons
// (opened via Spotlight/taskbar/AI) but remain in APPS for Spotlight + getDefaultSize.
export const DESKTOP_APPS = Object.entries(APPS)
  .filter(([key]) => !['PdfViewer', 'ImageViewer', 'Mail', 'KnownBook'].includes(key))
  .map(([key, value]) => ({
    name: value.name,
    app: key,
    icon: value.icon
...
export const GESTURE_ACTION_CONFIG = {
  ...
  gestures: {
    ...
    THREE_FINGERS: { action: 'openApp', target: 'Translator', label: 'Open Translator' },
