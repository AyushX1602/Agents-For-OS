...
function Calculator() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Only intercept if the Calculator window is currently focused
      const focusedApp = useWindowStore.getState().windows.find(w => w.focused)?.app
      if (focusedApp !== 'Calculator') return

      const key = e.key
      const consumed = /^[0-9]$/.test(key) || '+-*/.='.includes(key) ||
  ...
  const handleOperator = (op) => {
    ...
    if (op === '%') {
      // Append '%' to both display and expression; it is converted to a
      // "divide by 100" at evaluation time (handleEquals). Previously this
      // pre-divided the display yet left a literal '%' in the expression,
      // which JS evaluated as MODULO (e.g. 200 × 50% gave 0 instead of 100).
  ...
  const handleEquals = () => {
    try {
      let expr = expression
        ...
        .replace(/÷/g, '/')
        .replace(/\^/g, '**')
        // Convert percentages to a fraction so '50%' => '(50/100)'. This makes
        // both standalone '50%' (=0.5) and '200*50%' (=100) behave as expected.
