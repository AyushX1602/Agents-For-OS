 ...
 */
function GridItem({ item, selected, onClick, onDoubleClick, onContextMenu, onDragStart, onDrop }) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`${isDir ? 'Folder' : 'File'}: ${item.name}`}
      aria-selected={isSelected}
      draggable
      onDragStart={(e) => onDragStart && onDragStart(item, e)}
      onDragOver={(e) => { if (isDir) { e.preventDefault(); setDragOver(true) } }}
      onDragLeave={() => setDragOver(false)}
      onDrop={(e) => { setDragOver(false); onDrop && onDrop(item, e) }}
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onDoubleClick(item) } }}
      className={`flex flex-col items-center p-3 rounded-lg cursor-pointer transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-os-accent ${
        dragOver
          ? 'bg-os-accent/30 border border-os-accent'
          : isSelected
 ...
 * @param {Function} props.onDoubleClick - Double click handler
 */
function ListItem({ item, selected, onClick, onDoubleClick, onContextMenu, onDragStart, onDrop }) {
  const isSelected = Array.isArray(selected) ? selected.includes(item.path) : selected === item.path
  const [dragOver, setDragOver] = React.useState(false)
  const isDir = item.type === 'directory'
  ...
  return (
    <div
      ...
      onDrop={(e) => { setDragOver(false); onDrop && onDrop(item, e) }}
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onDoubleClick(item) } }}
      className={`flex items-center gap-3 px-3 py-2 cursor-pointer transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-os-accent ${
        dragOver
          ? 'bg-os-accent/30'
          : isSelected
            ? 'bg-os-accent/20'
 ...
 */
function FileGrid({ items, viewMode = 'grid', selected, onSelect, onOpen, onContextMenu, onDragStart, onDrop }) {
