...
function Translator() {
  ...
  // ── Translate — tries Sarvam first, falls back to MyMemory ──
  const translate = useCallback(async (text, overrides = {}) => {
    ...
    try {
      ...
      // Try Sarvam (server-side, works when SARVAM_ENABLED=true)
      let translated = null
      try {
        const sarvamRes = await fetch('/api/voice/translate', {
          method:  'POST',
        ...
        if (sarvamData.text && !sarvamData.fallback) {
          translated = sarvamData.text
        }
      ...
      setHistory(prev => [{
        source: text.trim(), result: translated, from, to,
  ...
  // Stop speech recognition if the window unmounts mid-listen so the mic is
  // released and we don't setState on an unmounted component.
  useEffect(() => () => {
      ...
      <div className="flex items-center gap-2 px-5 py-3 flex-shrink-0">
        ...
        <div className="flex-1">
          <select
            ...
            className="w-full px-3 py-2 bg-os-surface border border-os-border rounded-lg text-sm text-os-text-primary outline-none focus:border-indigo-500/50 focus-visible:ring-2 focus-visible:ring-indigo-500 cursor-pointer appearance-none"
          >
            {LANGUAGES.map(l => (
        ...
        <button
          ...
          className="w-10 h-10 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center hover:bg-indigo-500/30 transition-all hover:scale-110 active:scale-95 flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
          title="Swap languages"
        >
          <span aria-hidden="true" className="material-symbols-outlined text-indigo-400 text-lg">swap_horiz</span>
        </button>
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-os-border flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="p-1.5 rounded-md hover:bg-os-surface text-os-text-secondary hover:text-os-text-primary transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                title="Clear"
              >
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-os-border flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 rounded-md hover:bg-os-surface text-os-text-secondary hover:text-os-text-primary transition-all disabled:opacity-20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                title="Listen"
              >
                <span aria-hidden="true" className="material-symbols-outlined text-[18px]">volume_up</span>
              </button>
              <button
