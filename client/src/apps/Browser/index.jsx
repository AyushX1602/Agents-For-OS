...
function Browser({ initialUrl, navSeq } = {}) {
  ...
  // Navigate to a URL passed by the AI (open_url action).
  // navSeq is bumped by windowStore on every re-open of the same window,
  // so this effect re-fires even when initialUrl doesn't change.
  useEffect(() => {
    if (initialUrl) {
      const t = setTimeout(() => openUrl(initialUrl), 120)
      return () => clearTimeout(t)
    }
  }, [initialUrl, navSeq]) // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="p-4 bg-os-surface/80 border-b border-os-border">
        ...
        <div className="flex justify-center gap-0.5 mt-2">
          ...
          <div className="relative group">
            <button aria-haspopup="true" className="flex items-center gap-1 px-2 py-1 text-os-text-secondary hover:text-os-text-primary hover:bg-os-surface-hover rounded-md transition-all text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              <Globe size={12} />
              <span>Favorites</span>
            </button>
            {/* Favorites Dropdown */}
            <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 w-64 bg-os-surface border border-os-border rounded-lg shadow-xl z-50 hidden group-hover:block group-focus-within:block">
                ...
                {favorites.map((fav) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-os-text-secondary hover:text-red-400 p-1 cursor-pointer rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 gap-2">
            {QUICK_LINKS[activeCategory]?.map((link) => (
              <button
                ...
                className="group flex flex-col items-center p-3 rounded-lg bg-os-surface hover:bg-os-surface-hover border border-os-border hover:border-primary/30 transition-all cursor-pointer hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              >
                <img
                  src={link.favicon}
                  alt=""
              ...
              {favorites.map((fav) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={`Remove ${fav.name} from favorites`}
                    onClick={(e) => { e.stopPropagation(); removeFavorite(fav.url) }}
                    className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 text-os-text-secondary hover:text-red-400 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                  >
                    <Trash2 size={10} />
                  </button>
                </div>
