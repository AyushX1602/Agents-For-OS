...
export function showToast(message, opts = {}) {
  // Respect DND — skip non-critical toasts when focus mode is on
  const isCritical = opts.type === 'alert' || opts.type === 'error' ||
    (message || '').toLowerCase().includes('sos')
  const dnd = useOsStore.getState?.()?.dndEnabled ?? false
  if (dnd && !isCritical) return

  // Sync to notification history if not skipped
  if (!opts.skipHistory) {
    const addNotif = useOsStore.getState?.()?.addNotification
    if (addNotif) {
      addNotif(message, opts.type || 'info', true /* skipToast */)
    }
  }
...
// Register showToast on useOsStore to avoid circular dependency
if (useOsStore) {
  useOsStore.showToast = showToast
}
