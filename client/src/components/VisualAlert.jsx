...
export default function VisualAlert() {
  const { notifications, visualAlertsEnabled, dndEnabled } = useOsStore()
  ...
  useEffect(() => {
    ...
    // Respect DND — only flash for critical types when DND is on
    const isCritical = latest.type === 'alert' || latest.type === 'error' ||
      (latest.message || '').toLowerCase().includes('sos')
    if (dndEnabled && !isCritical) return
  ...
  }, [notifications, visualAlertsEnabled, dndEnabled])
