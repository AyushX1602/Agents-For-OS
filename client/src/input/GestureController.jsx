...
import { loadMediaPipeHandsCtor } from './loadMediaPipeHands'

const CONFIG = GESTURE_TIMING_CONFIG

// ── Constants ─────────────────────────────────────────────────────────────────
const PINCH_THRESH = 0.38       // pinch_ratio < this  → pinch (tighter than before)
const SWIPE_DX     = 0.15       // normalized horizontal hand movement for swipe
const SWIPE_MS     = 700
const SWIPE_MAX_DY = 0.18       // reject diagonal/vertical hand waves
const SWIPE_MIN_SPEED = 0.35    // normalized camera-width units per second
const SWIPE_ARMING_DX = 0.08    // suppress static false positives while moving
...
function classify(landmarks) {
  ...
  const curledForThumbCount = [
    fingerCurledForThumb(idxTip, idxPip, idxMcp),
    fingerCurledForThumb(midTip, midPip, midMcp),
    fingerCurledForThumb(ringTip, ringPip, ringMcp),
    fingerCurledForThumb(pinkyTip, pinkyPip, pinkyMcp)
  ].filter(Boolean).length
  const mostlyFingersCurled = allFingersCurled || curledForThumbCount >= 3
  const thumbDeltaY = thumbTip.y - thumbMcp.y
  const thumbUpRaw = thumbDeltaY < -palm * 0.45 && thumbTip.y < idxMcp.y - palm * 0.15
  const thumbDownRaw = thumbDeltaY > palm * 0.45 && thumbTip.y > idxMcp.y + palm * 0.15

  const extCount = [idxExt, midExt, ringExt, pinkyExt].filter(Boolean).length
  const threeNonThumb = extCount === 3
  const threeWithThumb = thumbExt && idxExt && midExt && !ringExt && !pinkyExt
  const loveYouSign = thumbExt && idxExt && !midExt && !ringExt && pinkyExt
  ...
  // 4. THREE / TRANSLATOR — accept common 3-finger variants plus the 🤟
  // legend sign, so the displayed shortcut matches what users naturally try.
...
export default function GestureController() {
  ...
  const onResults = useCallback((results) => {
    ...
    // Track palm-center motion for swipe. Keep tracking through brief
    // pinch/three misreads because finger landmarks flicker while the hand moves.
    let horizontalMotion = false
    if (features?.palmCenter) {
      const now = Date.now()
      swipeTrackRef.current.push({ x: features.palmCenter.x, y: features.palmCenter.y, t: now, gesture })
      swipeTrackRef.current = swipeTrackRef.current.filter(p => now - p.t <= SWIPE_MS)
      ...
      if (tr.length >= 4) {
        ...
        const mostlySameDirection = tr.slice(1).filter((p, i) => {
          const step = p.x - tr[i].x
          return Math.sign(step) === Math.sign(dx) || Math.abs(step) < 0.015
        }).length >= tr.length - 2
        const thumbPoseInTrack = tr.some(p => p.gesture === 'thumb_up' || p.gesture === 'thumb_down')

        horizontalMotion =
          !thumbPoseInTrack &&
          Math.abs(dx) > SWIPE_ARMING_DX &&
          Math.abs(dy) < SWIPE_MAX_DY &&
          speed > SWIPE_ARMING_SPEED
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // Load MediaPipe Hands from local /mediapipe-hands/ (no CDN dependency).
        // The UMD bundle is no longer in index.html — always use the npm package.
        const HandsCtor = await loadMediaPipeHandsCtor()
  ...
  return (
    <>
      ...
      {/* Gesture legend — collapsed by default, expand on click */}
      <div className="fixed top-20 right-5 z-[9997]">
        <button
          onClick={() => setLegendOpen(o => !o)}
          className="glass-panel rounded-full w-7 h-7 flex items-center justify-center text-[13px] text-fg-mut hover:text-fg transition-colors shadow-sm"
          title="Show gesture reference"
        >
          ?
        </button>
        {legendOpen && (
          <div className="absolute top-9 right-0 glass-panel rounded-2xl px-3 py-2 text-[11px] text-fg w-[200px]"
