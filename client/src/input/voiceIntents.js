...
export async function executeIntent({ intent, args }, deps) {
  ...
  switch (intent) {
    case 'reminder.create': {
      const combinedPhrase = `${args.time || ''} ${args.title || ''}`.toLowerCase()
      let daysMask = '1111111'
      if (combinedPhrase.includes('weekday') || combinedPhrase.includes('monday to friday') || combinedPhrase.includes('monday through friday')) {
        daysMask = '0111110'
      } else if (combinedPhrase.includes('weekend')) {
        daysMask = '1000001'
      } else if (combinedPhrase.includes('every day') || combinedPhrase.includes('daily') || combinedPhrase.includes('each day') || combinedPhrase.includes('everyday')) {
        daysMask = '1111111'
      } else {
        ...
        for (let i = 0; i < 7; i++) {
          if (combinedPhrase.includes(daysList[i])) {
            mask += '1'
            hasSpecificDay = true
          } else {
            mask += '0'
        ...
        if (hasSpecificDay) daysMask = mask
      }

      const cleanTime = (args.time || '')
        .replace(/\b(every|on|each)?\s*(sunday|monday|tuesday|wednesday|thursday|friday|saturday|weekdays|weekends|daily|everyday)s?\b/gi, '')
        .replace(/\b(every day)\b/gi, '')
        .trim()

      const t = parseTimePhrase(cleanTime)
      ...
      try {
        ...
        return `Okay. I'll remind you to ${args.title} at ${fmtTime12(t)} ${dayDesc}.`
      } catch (err) {
